...
class RnnModel:
    ...
    def __init__(self, params):
        ...
        # rango added 23.10.20########################################
        # Batch norm params
        #self.bn_layer = tf.layers.BatchNormalization(name='bn_layer') if self.net_name == 'rnn_propensity_weighted' else None
        self.bn_layer = None
        ...
        for i in range(num_samples):
            ...
            if self.softmax_size != 0:
                ...
                if self.bn_layer is not None:
                    logits = self.bn_layer(logits, training=False)
                # ############################################################
    ...
    def get_training_graph(self,
        ...
        for chunk_size in chunk_sizes:
            ...
            if self.softmax_size !=0:
                ...
                # rango added 23.10.20########################################
                if self.bn_layer is not None:
                    logits = self.bn_layer(logits, training=True)
                # ############################################################
                output = self.output_activation(logits)
        ...
        # handles = {'loss': loss,
        #           'optimiser': optimiser}
        # output more things to debug
        handles = {'loss': loss,
                  'optimiser': optimiser,
                  'numerator': tf.reduce_sum(tf.square(predictions - output_minibatch) * active_entries * weights)}

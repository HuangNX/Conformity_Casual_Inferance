...
class FactorModel:
    def __init__(self, params, hyperparams):
        # 使用tf.compat.v1模块来使用Session
        tf.compat.v1.disable_eager_execution()  # 禁用eager excution
        ...
        self.target_treatments = tf.compat.v1.placeholder(tf.float32, [None, self.max_sequence_length, self.num_treatments])
    ...
    def build_confounders(self, trainable_state=True):
        ...
        if trainable_state:
            init_state = tf.compat.v1.get_variable(name='init_cell',
        ...
        rnn_output, _ = tf.compat.v1.nn.dynamic_rnn(
            autoregressive_cell,
            self.rnn_input,
    ...
    def build_treatment_assignments(self):
        ...
        for treatment in range(self.num_treatments):
            treatment_network_layer = tf.compat.v1.layers.dense(self.multitask_input, self.fc_hidden_units,
                                                      name='treatment_network_%s' % str(treatment),
                                                      activation=tf.nn.leaky_relu)

            treatment_output = tf.compat.v1.layers.dense(treatment_network_layer, 1, activation=tf.nn.sigmoid,
    ...
    def train(self, dataset_train, dataset_val, verbose=False):
        ...
        self.treatment_realizations = tf.compat.v1.distributions.Bernoulli(probs=self.treatment_prob_predictions).sample()
